...
<body>
    ...
    <div class="sidebar">
        <div class="bar1">
           <a href="home.php"><li><i class="bi bi-house-fill"></i></li></a> <br>
            <a href="search.php"><li><i class="bi bi-search"></li></i></a>
        </div>
        <div class="bar2">
            <div class="bar21">
                <a href="library.php"><li><i class="bi bi-layers-fill"></i></li></a><br>
                <a href="likedhem.php"><li> <i class="bi bi-heart-fill"></i></li></a><br>
                <a href="queue.php"> <i class="fa-solid fa-guitar" style="color: #dbf5f1"></i></li></a>
            </div>
    ...
    <div class="mainbar">
        ...
        <div class="heart">
            <div class="box">
            <i class="fa-solid fa-guitar" style="color: #dbf5f1"></i>
            </div>
        </div>
        <div class="liked">
            <h4>playlist</h4>
...
const likedplaylist = JSON.parse(localStorage.getItem('myqueue'));
console.log(likedplaylist);
let loc = document.getElementById("loc"); // Get the container element to append the divs.

// ... Your existing code ...

// Make sure 'likedplaylist' is an array and not empty before looping through it.
  ...
  for (let m = 0; m < likedplaylist.length; m++) {
    ...
    let p = document.createElement("p");
            p.addEventListener("click", function () {
                playmysong(likedplaylist[m]);
            });
            let p2= document.createElement("p");
            p2.addEventListener("click", function () {
                removeFromLikedPlaylist(likedplaylist[m]);
            });
    let i = document.createElement("i");
    i2.className = "bi bi-trash-fill";
    div.className = "box";
    img.src = songlist[likedplaylist[m]].image;
    h5.innerHTML = songlist[likedplaylist[m]].name;
...
function removeFromLikedPlaylist(songId) {
    // Get the liked playlist from local storage
    let likedSongs = JSON.parse(localStorage.getItem("myqueue")) || [];

    // Find the index of the songId in the liked playlist
    const index = likedSongs.indexOf(songId);

    // If the songId is present in the liked playlist, remove it
    if (index !== -1) {
        likedSongs.splice(index, 1);

        // Update the liked playlist in local storage
        localStorage.setItem("myqueue", JSON.stringify(likedSongs));
    }
}
function loop()
{if(audio.loop)
